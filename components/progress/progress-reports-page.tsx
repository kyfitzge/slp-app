...
function ReportCard({
  ...
  const excerpt = report.summaryText?.trim().slice(0, 140);
  ...
  return (
    <button
      onClick={onLoad}
      className={cn(
        "group relative w-full rounded-lg border p-3 text-left text-xs transition-colors",
        isActive
    ...
    >
      ...
      {hasExcerpt ? (
        <p className="text-muted-foreground leading-relaxed line-clamp-2">
          {excerpt}{(report.summaryText?.length ?? 0) > 140 ? "…" : ""}
        </p>
      ) : (
    ...
    <div className="flex flex-col h-full max-w-[1600px]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-4 flex-1 min-h-0">
        ...
        <div className="flex flex-col gap-4 min-h-0 overflow-hidden">
            ...
            <CardHeader className="pb-3 shrink-0">
              <div className="flex items-center justify-between">
                <CardTitle className="text-sm font-semibold flex items-center gap-2">
                  ...
                  {selectedStudent
                    ? `${selectedStudent.firstName} ${selectedStudent.lastName}'s Reports`
                    : "Past Reports"}
                </CardTitle>
                {selectedStudentId && (
                  <Button
              ...
              {!selectedStudentId ? (
                ...
                <div className="py-6 text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Use the editor below to create the first report for this student.
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 max-h-[230px] overflow-y-auto pr-1">
          ...
          {/* ── Editor Draft Box ── */}
          <div className="flex flex-col min-h-0 flex-1 overflow-hidden rounded-xl border bg-card">
            {!selectedStudentId ? (
